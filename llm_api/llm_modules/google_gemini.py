...
- If the response is off-topic, focus on the lecture content.
- After three off-topic responses, ask a targeted question from an unaddressed topic.
- Ask follow-up questions if the response is unique, interesting, incomplete, or needs clarification; otherwise, move to a new topic.
- Maintain a professional tone and track conversation history.
...
- Tailor questions to a candidate with experience level: "{experience}".
- Use a "{tone}" tone.
...
def get_next_response(messages):
    ...
    for msg in messages:
        ...
        content = msg["content"]
        if role == "model" and content.startswith('{'):
            try:
    ...
    response = client.generate_content(
        history[-1]["parts"][0] if history else "Start the interview.",
        generation_config={
